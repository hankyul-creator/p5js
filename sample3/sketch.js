...
function draw(){
  ...
  fill(255, 220, 180);
  noStroke();
  ellipse(220, 150, 30, 50); // 얼굴 왼쪽 옆, 타원 형태로 귀

  // 오른쪽 귀
  ellipse(380, 150, 30, 50); // 얼굴 오른쪽 옆, 타원 형태로 귀
  ...
  noStroke();  
  if (mouseIsPressed){
    ellipse(265+(mouseX-300)/100, 150+(mouseY-200)/100, 15, 14);} else{
  ...
  noStroke();
  if (mouseIsPressed){
    ellipse(335+(mouseX-300)/100, 150+(mouseY-200)/100, 15, 14);} else{
  ...
  if(x>=300){
    ...
    stroke(0);          // 선 색: 검은색
    strokeWeight(3);
    arc(x+7.5, y-7.5, 10, 10, PI, 3*HALF_PI);
  } 
}
